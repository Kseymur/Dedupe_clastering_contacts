...
if __name__ == '__main__':
    ...
    if os.path.exists(settings_file):  # переходим в эту ветку, если уже есть обученная модель и надо просто получить дубликаты
        ...
        if os.path.exists(training_file):  # если уже есть какой-то тренировочный файл, то загружаем его без первичного обучения
            train_or_not = get_input(
            "У вас уже есть файл с размеченными экземплярами. Запустим поиск дублей с его помощью? (y/n): ",
            ['y', 'n']
        )
            if train_or_not.lower() == 'y':
                with open(training_file) as tf:
                    deduper.prepare_training(data_for_training, tf)
            else:
                delete_or_not = get_input(
                    "Удалить файл и начать обучение заново? (y/n): ",
                    ['y', 'n']
                )
                if delete_or_not.lower() == 'y':
                    ...
                    deduper.prepare_training(
                        data_for_training)  # подготавливаем модель к первичной разметке, нужно подождать. Чем
                    # больше полей было указано для обучения, тем больше времени займет подготовка к обучению
                    print(
                        'Сейчас вам нужно будет сравнивать контакты и отвечать на вопрос: дубли это или нет. Введите y, если «да», n, если «нет», u, если вы не уверены. Когда закончите, введите f. PS: оценить стоит хотя бы несколько десятков пар, как минимум одну. Если видите значения None, всё равно попытайтесь их оценить')
